...
public class CSharpBuilder extends ASTVisitor {
	...
	public CSharpBuilder() {
		...
		_compilationUnit.addUsing(new CSUsing("System"));
	}
	...
	private boolean buildEnumType(final AbstractTypeDeclaration typeNode) {
		...
		typeNode.accept(new ASTVisitor() {
			public boolean visit(EnumConstantDeclaration node) {
				if (!(typeNode instanceof EnumDeclaration)) {
					unsupportedConstruct(node,
							"Only enum types can contain enum constants.");
					return false;
				}
			...
			public boolean visit(VariableDeclarationFragment node) {
				if (typeNode instanceof EnumDeclaration) {
					unsupportedConstruct(node,
							"Enum types cannot contain variable declarations.");
					return false;
				}
	...
	private void processNonStaticNestedTypeDeclaration(
			AbstractTypeDeclaration node) {
		if (!(node instanceof TypeDeclaration)) {
			unsupportedConstruct(node, "Declarations of type "
					+ node.getClass().getName()
					+ " cannot be non-static nested classes.");
		}

		new NonStaticNestedClassBuilder(this, (TypeDeclaration) node);
	}

	protected CSTypeDeclaration processTypeDeclaration(
			AbstractTypeDeclaration node) {
		...
		if (_configuration.shouldMakePartial(node.getName()
	...
	private void autoImplementCloneable(AbstractTypeDeclaration node,
			CSTypeDeclaration type) {
	...
	private void mapSuperTypes(AbstractTypeDeclaration node,
			CSTypeDeclaration type) {
	...
	private void processConversionJavadocTags(AbstractTypeDeclaration node,
			CSTypeDeclaration type) {
	...
	private CSTypeDeclaration mapAuxillaryTypeDeclaration(
		...
		if (!(node instanceof TypeDeclaration)
				&& !(node instanceof EnumDeclaration)) {
			unsupportedConstruct(node,
					"Cannot map auxillary type declaration for node.");
		}
	...
	private CSTypeDeclaration auxillaryTypeDeclarationFor(
			AbstractTypeDeclaration node) {
	...
	private CSTypeDeclaration typeDeclarationFor(AbstractTypeDeclaration node,
		...
		if (node instanceof TypeDeclaration
			...
			if (valid && auxillary) {
				return null;
					...
					|| (!auxillary && _configuration
							.separateInterfaceConstants())) {
				boolean overriddenName = !typeName.equals(node.getName()
						.toString());
				if (overriddenName) {
				...
				return new CSClass(processInterfaceConstantsName(node),
						CSClassModifier.Static);
			} else {
				warningHandler().warning(node,
						"Java interface converted to C# class.");
		...
		if (node instanceof TypeDeclaration) {
			...
			unsupportedConstruct(node,
					"cannot determine modifiers for type node");
	...
	private CSTypeDeclaration checkForMainType(AbstractTypeDeclaration node,
			CSTypeDeclaration type) {
		if (isMainType(node)) {
			...
			if (_configuration.paramCountFileNames()) {
				if (type.typeParameters().size() > 0) {
					name += "`"
							+ Integer.toString(type.typeParameters().size());
				}
			}
	...
	private void mapSuperClass(AbstractTypeDeclaration node,
			CSTypeDeclaration type) {
	...
	private boolean isLegacyTestFixtureClass(ITypeBinding type) {
		return (_configuration.junitConversion() && type.getQualifiedName()
		...
		for (CSMember m : type.members()) {
			if (m instanceof CSMethod) {
				...
				for (CSAttribute at : met.attributes()) {
					if (at.name().equals("Test")
							|| at.name().equals("NUnit.Framework.Test"))
						return true;
				}
			}
		}
	...
	private boolean handledExtends(AbstractTypeDeclaration node,
			CSTypeDeclaration type) {
		if (!(node instanceof TypeDeclaration)
				&& !(node instanceof EnumDeclaration)) {
	...
	private void mapSuperInterfaces(AbstractTypeDeclaration node,
			CSTypeDeclaration type) {
		List superInterfaceTypes;
		if (node instanceof TypeDeclaration) {
			superInterfaceTypes = ((TypeDeclaration) node)
					.superInterfaceTypes();
		} else if (node instanceof EnumDeclaration) {
			superInterfaceTypes = ((EnumDeclaration) node)
					.superInterfaceTypes();
		} else {
			unsupportedConstruct(node,
					"cannot map super interfaces for type node");
			throw new UnsupportedOperationException("shouldn't be reachable");
		}
	...
	private void mapMembers(AbstractTypeDeclaration node,
			CSTypeDeclaration type, CSTypeDeclaration auxillaryType) {
		if (!(node instanceof TypeDeclaration)
				&& !(node instanceof EnumDeclaration)) {
	...
	private boolean isAttributeAnnotation(TagElement element) {
		return element.getTagName()
				.equals(SharpenAnnotations.SHARPEN_ATTRIBUTE);
	}
	...
	private void processAttribute(CSMember member, TagElement element) {
		String attrType = mappedTypeName(JavadocUtility
				.singleTextFragmentFrom(element));
	...
	private void processPartialTagElement(AbstractTypeDeclaration node,
	...
	private CSDocNode mapSingleTextElementCodeTagNode(CSMember member,
			TagElement element) {
	...
	private CSDocNode mapTagWithCRef(CSMember member, String tagName,
			TagElement element) {
	...
	private CSDocNode invalidTagWithCRef(CSMember member,
			final ASTNode linkTarget, String tagName, TagElement element) {
	...
	private void collectFragments(CSMember member, CSDocTagNode node,
			List fragments, int index) {
		for (int i = index; i < fragments.size(); ++i) {
			node.addFragment(mapTagElementFragment(member,
					(ASTNode) fragments.get(i), false));
		}
	}
	...
	private CSDocNode createTagNode(CSMember member, String tagName,
			TagElement element, boolean escapeText) {
	...
	public boolean visit(FieldDeclaration node) {
		...
		if (((VariableDeclarationFragment) node.fragments().get(0))
				.resolveBinding().getDeclaringClass().isInterface())
	...
	private CSField mapFieldDeclarationFragment(FieldDeclaration node,
		...
		if (fragment.getExtraDimensions() > 0) {
			fieldType = new CSArrayTypeReference(fieldType,
	...
	private boolean isConstField(FieldDeclaration node,
		...
		return Modifier.isFinal(node.getModifiers())
				&& isSupportedConstantType(node.getType())
				&& hasConstValue(fragment)
	...
	private String couldBeCSProperty(MethodDeclaration node) {
		if (node.getName().toString().startsWith("get")
				|| node.getName().toString().startsWith("set")) {
			String propertyName = node.getName().toString().substring(3);
			String lowerCaseFirstCharacter = propertyName.substring(0, 1)
					.toLowerCase() + propertyName.substring(1);
			CSMember member = _currentType.getMember(lowerCaseFirstCharacter);
			if (member != null) {
				return propertyName;
			}
		}
		if (node.getName().toString().startsWith("is")) {
			String propertyName = node.getName().toString().substring(2);
			String lowerCaseFirstCharacter = propertyName.substring(0, 1)
					.toLowerCase() + propertyName.substring(1);
			CSMember member = _currentType.getMember(lowerCaseFirstCharacter);
			if (member != null) {
	...
	public boolean visit(MethodDeclaration node) {
		...
		String propertyName = couldBeCSProperty(node);
		if (propertyName != null) {
			processPropertyDeclaration(node, propertyName);
			return false;
		}
		...
		if (_configuration.junitConversion()
			...
			if (isLegacyTestFixtureClass(node.resolveBinding()
				...
				if (method.name().equals("SetUp")) {
					...
					method.addAttribute(new CSAttribute(
	...
	private void mapMethodParts(MethodDeclaration node, CSMethodBase method) {
		...
		if (!node.isConstructor() && overriden != null) {
			...
			if (vis == CSVisibility.ProtectedInternal
					&& !overriden.getDeclaringClass().isFromSource())
	...
	private void mapParameter(SingleVariableDeclaration parameter,
		...
		if (method instanceof CSMethod) {
			...
			if (ta.length > 0 && ta[0].getName().startsWith("?")) {
				...
				CSTypeReference tr = new CSTypeReference(
						mappedTypeName(vb.getType()));
				tr.addTypeArgument(new CSTypeReference(genericArg));
				method.addParameter(new CSVariableDeclaration(identifier(vb
						.getName()), tr));
				return;
			}
		}
	...
	ITypeBinding mapTypeParameterExtendedType(ITypeBinding tb) {
		...
		if (superc != null
				&& !superc.getQualifiedName().equals("java.lang.Object")
	...
	private TagElement effectiveAnnotationFor(BodyDeclaration node,
	...
	private CSVariableDeclaration createVariableDeclaration(
		...
		return new CSVariableDeclaration(identifier(name),
	...
	public boolean visit(final WhileStatement node) {
		consumeContinueLabel(new Function<CSBlock>() {
			public CSBlock apply() {
				CSWhileStatement stmt = new CSWhileStatement(
	...
	public boolean visit(TryStatement node) {
		...
		for (Object resource : node.resources()) {
			...
			CSUsingStatement using = new CSUsingStatement(
					expression.getStartPosition(), popExpression());
		...
		for (Object o : node.catchClauses()) {
			...
			if (!_configuration.isIgnoredExceptionType(qualifiedName(clause
					.getException().getType().resolveBinding()))) {
	...
	public boolean visit(BreakStatement node) {
		...
		if (labelName != null) {
			addStatement(new CSGotoStatement(node.getStartPosition(),
					breakLabel(labelName.getIdentifier())));
			return false;
		}
	...
	public boolean visit(NumberLiteral node) {
		...
		if (expectingType("byte") && token.startsWith("-")) {
			...
			try {
				...
				if (n != 0)
					literal = new CSNumberLiteralExpression("0x"
	...
	public boolean visit(StringLiteral node) {
		...
		if (value != null && value.length() == 0) {
			...
			pushExpression(new CSStringLiteralExpression(
					fixEscapedNumbers(node.getEscapedValue())));
		}
		return false;
	}
		...
		for (int n = 0; n < literal.length(); n++) {
			if (literal.charAt(n) == '\\') {
				...
				while (i < literal.length()
						&& Character.isDigit(literal.charAt(i)))
					i++;
				if (i != n + 1) {
	...
	public boolean visit(CharacterLiteral node) {
		...
		if (expectingType("byte")) {
			expr = new CSCastExpression(new CSTypeReference("byte"),
					new CSParenthesizedExpression(expr));
		}
		pushExpression(expr);
		return false;
	}
	...
	private boolean expectingType(String name) {
		return (_currentExpectedType != null && _currentExpectedType.getName()
				.equals(name));
	}
	...
	public boolean visit(ArrayCreation node) {
		ITypeBinding saved = pushExpectedType(node.getType().getElementType()
				.resolveBinding());
		...
		if (isImplicitelyTypedArrayInitializer(node)) {
			CSArrayCreationExpression ace = new CSArrayCreationExpression(
					mappedTypeReference(node.resolveTypeBinding()
							.getComponentType()));
			ITypeBinding saved = pushExpectedType(node.resolveTypeBinding()
					.getElementType());
	...
	public boolean visit(final ForStatement node) {
		consumeContinueLabel(new Function<CSBlock>() {
			public CSBlock apply() {
				...
				CSForStatement stmt = new CSForStatement(
						node.getStartPosition(),
						mapExpression(node.getExpression()));
			...
			if (ASTNode.SWITCH_CASE == element.getNodeType()) {
				if (null == current) {
					if (_currentBlock != null) {
						...
						CSStatement lastStmt = stats.size() > 0 ? stats
								.get(stats.size() - 1) : null;
				...
				if (sc.isDefault()) {
					...
					if (openCaseBlock != null)
						openCaseBlock.addStatement(new CSGotoStatement(
								Integer.MIN_VALUE, "default"));
				} else {
					ITypeBinding stype = pushExpectedType(switchType);
					CSExpression caseExpression = mapExpression(sc
							.getExpression());
					current.addExpression(caseExpression);
					popExpectedType(stype);
					if (openCaseBlock != null)
						openCaseBlock.addStatement(new CSGotoStatement(
								Integer.MIN_VALUE, caseExpression));
				}
				openCaseBlock = null;
			} else {
				element.accept(this);
	...
	public boolean visit(PrefixExpression node) {
		CSExpression expr;
		expr = new CSPrefixExpression(node.getOperator().toString(),
				mapExpression(node.getOperand()));
		if (expectingType("byte")
				&& node.getOperator() == PrefixExpression.Operator.MINUS) {
	...
	public boolean visit(InfixExpression node) {
		...
		String type = node.getLeftOperand().resolveTypeBinding()
				.getQualifiedName();
		if (node.getOperator() == InfixExpression.Operator.RIGHT_SHIFT_UNSIGNED) {
			if (type.equals("byte")) {
				pushExpression(new CSInfixExpression(">>", left, right));
			} else {
				CSExpression cast = new CSCastExpression(new CSTypeReference(
						"u" + type), left);
				cast = new CSParenthesizedExpression(cast);
				CSExpression shiftResult = new CSInfixExpression(">>", cast,
						right);
				shiftResult = new CSParenthesizedExpression(shiftResult);
				pushExpression(new CSCastExpression(new CSTypeReference(type),
						shiftResult));
			}
			return false;
		}
		if (type.equals("byte")
				&& (node.getOperator() == InfixExpression.Operator.LESS || node
						.getOperator() == InfixExpression.Operator.LESS_EQUALS)) {
	...
	public boolean visit(AssertStatement node) {
		...
		CSExpression assertMethod = new CSMemberReferenceExpression(
				new CSTypeReference("System.Diagnostics.Debug"), "Assert");
		addStatement(new CSExpressionStatement(node.getStartPosition(),
				new CSMethodInvocationExpression(assertMethod,
						args.toArray(new CSExpression[args.size()]))));
		return false;
	}
	...
	public boolean visit(EnumConstantDeclaration node) {
		...
		ITypeBinding fieldType = ((EnumDeclaration) node.getParent())
				.resolveBinding();
		CSTypeReferenceExpression typeName = mappedTypeReference(fieldType);
		CSVisibility visibility = mapVisibility(node);
		...
		if (null == mappedConstructor) {
			...
			if (mappedName.length() == 0) {
				...
				initializer = new CSMethodInvocationExpression(
						new CSReferenceExpression(methodName(mappedName)));
			}
		}

		mapArguments(initializer, node.arguments());

		CSField field = new CSField(fieldName(node), typeName, visibility,
				initializer);
		field.addModifier(CSFieldModifier.Static);
		field.addModifier(CSFieldModifier.Readonly);
		mapAnnotations(node, field);
		...
		if (node.getOperator() == Assignment.Operator.RIGHT_SHIFT_UNSIGNED_ASSIGN) {
			...
			if (type == "byte") {
				...
				CSExpression shiftResult = new CSInfixExpression(">>", cast,
						mapExpression(rhs));
				shiftResult = new CSParenthesizedExpression(shiftResult);
				shiftResult = new CSCastExpression(new CSTypeReference(type),
						shiftResult);
				pushExpression(new CSInfixExpression("=", mappedLhs,
						shiftResult));
			}
		} else {
	...
	private CSExpression castIfNeeded(ITypeBinding expectedType,
		...
		if (!_configuration.mapIteratorToEnumerator()
				&& expectedType.getName().startsWith("Iterable<")
				&& isGenericCollection(actualType)) {
			return new CSMethodInvocationExpression(
					new CSMemberReferenceExpression(expression, "AsIterable"));
		}
		if (expectedType != actualType
				&& isSubclassOf(expectedType, actualType))
	...
	private boolean isGenericCollection(ITypeBinding t) {
		return t.getName().startsWith("List<")
				|| t.getName().startsWith("Set<");
	}
	...
	private void processMacroInvocation(MethodInvocation node) {
		final MethodDeclaration declaration = (MethodDeclaration) declaringNode(node
				.resolveMethodBinding());
	...
	private void processOrdinaryMethodInvocation(MethodInvocation node) {
		...
		if ((method.getModifiers() & Modifier.STATIC) != 0
				...
				&& node.getExpression() != null)
			targetExpression = mappedTypeReference(node.getExpression()
					.resolveTypeBinding());
	...
	private String resolveTargetMethodName(CSExpression targetExpression,
			MethodInvocation node) {
		final IMethodBinding method = staticImportMethodBinding(node.getName(),
				_ast.imports());
	...
	private void processEnumOrdinalMethodInvocation(MethodInvocation node) {
		...
		pushExpression(new CSCastExpression(new CSTypeReference("int"),
				new CSParenthesizedExpression(exp)));
	}

	private void processEnumNameMethodInvocation(MethodInvocation node) {
		CSExpression exp = mapExpression(node.getExpression());
		pushExpression(new CSMethodInvocationExpression(
				new CSMemberReferenceExpression(exp, "ToString")));
	}
	...
	private void mapMethodInvocationArguments(CSMethodInvocationExpression mie,
		...
		for (int i = 0; i < arguments.size(); ++i) {
			...
			if (i < originalTypes.length
				...
				addArgument(mie, arg, i < actualTypes.length ? actualTypes[i]
						: null);
			}
		}
	...
	private void adjustJUnitArguments(CSMethodInvocationExpression mie,
			MethodInvocation node) {
		if (!_configuration.junitConversion())
			return;
		...
		if (t.getQualifiedName().equals("junit.framework.Assert")
			...
			if (method.equals("assertSame")) {
				...
				if (useEquals) {
					CSReferenceExpression mref = (CSReferenceExpression) mie
							.expression();
	...
	private void processEventSubscription(MethodInvocation node) {
		...
		final MethodDeclaration addListener = (MethodDeclaration) declaringNode(node
				.resolveMethodBinding());
		...
		final MethodDeclaration eventDeclaration = (MethodDeclaration) declaringNode(eventInvocation
				.resolveMethodBinding());
	...
	private void processMappedMethodInvocation(MethodInvocation node,
		...
		if (mapping.kind != MemberKind.Method) {
			...
			if (binding != originalBinding
					&& originalBinding.getReturnType() != binding
							.getReturnType()
					&& !(node.getParent() instanceof ExpressionStatement))
				target = new CSParenthesizedExpression(new CSCastExpression(
						mappedTypeReference(originalBinding.getReturnType()),
						target));
	...
	public boolean visit(SimpleName node) {
		if (isTypeReference(node)) {
			...
			IVariableBinding vb = b instanceof IVariableBinding ? (IVariableBinding) b
					: null;
			if (vb != null) {
				...
				if (cls != null) {
					if (isStaticImport(vb, _ast.imports())) {
						...
						pushExpression(new CSMemberReferenceExpression(
								mappedTypeReference(cls), ident));
						return false;
					} else if (_configuration.separateInterfaceConstants()
							&& cls.isInterface() && ident.indexOf('.') == -1) {
						pushExpression(new CSMemberReferenceExpression(
								mappedAuxillaryTypeReference(cls), ident));
						return false;
					}
				}
			}
		...
		if (_configuration.separateInterfaceConstants()) {
			...
			if (binding instanceof ITypeBinding) {
				...
				if (typeBinding.isInterface()) {
					pushExpression(new CSMemberReferenceExpression(
							mappedAuxillaryTypeReference(typeBinding), name));
					return;
				}
			}
		}
	...
	CSMethodModifier mapMethodModifier(MethodDeclaration method) {
		if (_currentType.isInterface()
				|| method.resolveBinding().getDeclaringClass().isInterface()) {
		...
		if (override) {
			return isFinal ? CSMethodModifier.Sealed
					: modifierIfNewAnnotationNotApplied(method,
	...
	private CSMethodModifier modifierIfNewAnnotationNotApplied(
			MethodDeclaration method, CSMethodModifier modifier) {
		return containsJavadoc(method, SharpenAnnotations.SHARPEN_NEW) ? CSMethodModifier.None
				: modifier;
	}
	...
	private IMethodBinding getOverridedMethod(IMethodBinding methodBinding) {
		...
		if (null != superclass) {
			IMethodBinding result = BindingUtils
					.findOverriddenMethodInHierarchy(superclass, methodBinding);
			if (null != result)
				return result;
		}

		if (!_configuration.separateInterfaceConstants()) {
			ITypeBinding[] baseInterfaces = methodBinding.getDeclaringClass()
					.getInterfaces();
			if (baseInterfaces.length == 1
					&& !isValidCSInterface(baseInterfaces[0])) {
				// Base interface generated as a class
				return BindingUtils.findOverriddenMethodInType(
						baseInterfaces[0], methodBinding);
			}
		}
	...
	private boolean isValidCSInterface(ITypeBinding type) {
		if (type.getTypeDeclaration().getQualifiedName()
	...
	void adjustVisibility(ITypeBinding memberType, CSMember member) {
		...
		if (typeVisibility == CSVisibility.Protected
	...
	protected CSTypeReferenceExpression mappedAuxillaryTypeReference(
			ITypeBinding type) {
	...
	protected CSTypeReferenceExpression mappedTypeReference(ITypeBinding type,
		...
		if (auxillary && !type.isInterface()) {
			throw new IllegalArgumentException(
					"Auxillary types are only available for interfaces.");
		}
	...
	private boolean isMappedToProperty(BodyDeclaration original) {
		...
		if (original instanceof MethodDeclaration) {
			...
			binding = ((AnnotationTypeMemberDeclaration) original)
					.resolveBinding();
		} else {
			throw new UnsupportedOperationException();
		}
