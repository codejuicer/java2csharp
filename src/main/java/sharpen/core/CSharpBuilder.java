...
public class CSharpBuilder extends ASTVisitor {
    ...
    private void processNonStaticNestedTypeDeclaration(AbstractTypeDeclaration node) {
        if (!(node instanceof TypeDeclaration)) {
            unsupportedConstruct(node, "Declarations of type " + node.getClass().getName()
    ...
    private void autoImplementCloneable(AbstractTypeDeclaration node, CSTypeDeclaration type) {
        ...
        clone.body()
            .addStatement(new CSReturnStatement(-1,
    ...
    private boolean isLegacyTestFixtureClass(ITypeBinding type) {
        return (_configuration.junitConversion()
                && type.getQualifiedName().equals("junit.framework.TestCase"));
    }
    ...
    private void mapMembers(AbstractTypeDeclaration node, CSTypeDeclaration type,
                            CSTypeDeclaration auxillaryType) {
        if (!(node instanceof TypeDeclaration) && !(node instanceof EnumDeclaration)) {
    ...
    private CSDocNode invalidTagWithCRef(CSMember member, final ASTNode linkTarget, String tagName,
                                         TagElement element) {
    ...
    private CSDocNode mapTagParam(CSMember member, TagElement element) {
        ...
        CSDocTagNode param = isPropertyNode(documentedNodeAttachedTo(element))
            ? new CSDocTagNode("value") : newCSDocTag(fixIdentifierNameFor(identifier(name), element));
    ...
    private CSField mapFieldDeclarationFragment(FieldDeclaration node, VariableDeclarationFragment fragment,
                                                CSTypeReferenceExpression fieldType,
                                                CSVisibility fieldVisibility) {
        if (fragment.getExtraDimensions() > 0) {
            fieldType = new CSArrayTypeReference(fieldType, fragment.getExtraDimensions());
        }
        CSField field = new CSField(fieldName(fragment), fieldType, fieldVisibility,
    ...
    private boolean isIgnoredAnnotation(Annotation m) {
        return _configuration
            .isIgnoredAnnotation(qualifiedName(m.resolveAnnotationBinding().getAnnotationType()));
    }
    ...
    private String couldBeCSProperty(MethodDeclaration node) {
        if (node.getName().toString().startsWith("get") || node.getName().toString().startsWith("set")) {
            String propertyName = node.getName().toString().substring(3);
            String lowerCaseFirstCharacter = propertyName.substring(0, 1).toLowerCase()
    ...
    private CSDeclarationExpression newVariableDeclarationExpression(final String name,
                                                                     final CSTypeReferenceExpression type,
    ...
    private CSProperty newPropertyFor(MethodDeclaration node, final String propName) {
        final CSTypeReferenceExpression propertyType = isGetter(node)
            ? mappedReturnType(node) : mappedTypeReference(lastParameter(node).getType());
    ...
    private void processMethodDeclaration(MethodDeclaration node) {
        ...
        if (_configuration.junitConversion()
            && isLegacyTestFixture(node.resolveBinding().getDeclaringClass())) {
    ...
    private void mapMethodParameters(MethodDeclaration node, CSMethod method) {
        for (Object o : node.parameters()) {
            ...
            if (isGenericRuntimeParameterIdiom(node.resolveBinding(), parameterType)) {
                ...
                method.body().addStatement(new CSDeclarationStatement(p
                    .getStartPosition(), new CSVariableDeclaration(identifier(p.getName()),
    ...
    private CSBlock createEventBlock(CSField backingField, String delegateMethod) {
        ...
        block.addStatement(new CSInfixExpression("=", new CSReferenceExpression(backingField
            .name()), new CSCastExpression(backingField
                .type(), new CSMethodInvocationExpression(new CSReferenceExpression(delegateMethod),
    ...
    private CSExpression getLockTarget(BodyDeclaration node) {
        return Modifier.isStatic(node.getModifiers())
            ? new CSTypeofExpression(new CSTypeReference(_currentType.name())) : new CSThisExpression();
    }
    ...
    private CSUncheckedExpression uncheckedCast(String type, CSExpression expression) {
        return new CSUncheckedExpression(new CSCastExpression(new CSTypeReference(type),
                                                              new CSParenthesizedExpression(expression)));
    }
    ...
    /**
     * Unfolds java multi array creation shortcut "new String[2][3][2]" into explicitly array creation "new
     * string[][][] { new string[][] { new string[2], new string[2], new string[2] }, new string[][] { new
    ...
    private CSArrayCreationExpression unfoldMultiArray(ArrayType type, List dimensions, int dimensionIndex) {
        ...
        if (dimensionIndex < lastIndex(dimensions) - 1) {
            for (int i = 0; i < length; ++i) {
                expression.initializer().addExpression(unfoldMultiArray((ArrayType)type.getComponentType(),
    ...
    public boolean visit(ArrayInitializer node) {
        if (isImplicitelyTypedArrayInitializer(node)) {
            CSArrayCreationExpression ace = new CSArrayCreationExpression(mappedTypeReference(node
                .resolveTypeBinding().getComponentType()));
    ...
    private boolean isSwitchCaseClosingStatement(CSStatement stmt) {
        return stmt instanceof CSThrowStatement || stmt instanceof CSReturnStatement
               || stmt instanceof CSBreakStatement || stmt instanceof CSGotoStatement
    ...
    public boolean visit(CastExpression node) {
        pushExpression(new CSCastExpression(mappedTypeReference(node.getType()),
                                            mapExpression(node.getExpression())));
    ...
    public boolean visit(AssertStatement node) {
        ...
        CSExpression assertMethod = new CSMemberReferenceExpression(new CSTypeReference("System.Diagnostics.Debug"),
                                                                    "Assert");
        addStatement(new CSExpressionStatement(node.getStartPosition(),
                                               new CSMethodInvocationExpression(assertMethod, args
                                                   .toArray(new CSExpression[args.size()]))));
        return false;
    }
    ...
    private CSExpression castIfNeeded(ITypeBinding expectedType, ITypeBinding actualType,
        ...
        if (!_configuration.mapIteratorToEnumerator() && expectedType.getName().startsWith("Iterable<")
            ...
            return new CSMethodInvocationExpression(new CSMemberReferenceExpression(expression,
                                                                                    "AsIterable"));
        }
    ...
    private void pushGetClassForTypeExpression(final CSTypeReferenceExpression typeName) {
        CSMethodInvocationExpression mie = new CSMethodInvocationExpression(new CSReferenceExpression(methodName(_configuration
    ...
    public boolean visit(SuperMethodInvocation node) {
        ...
        IMethodBinding binding = originalMethodBinding(node.resolveMethodBinding());
        Configuration.MemberMapping mapping = mappingForInvocation(node, binding);
        CSExpression target = new CSMemberReferenceExpression(new CSBaseExpression(),
    ...
    private void processOrdinaryMethodInvocation(MethodInvocation node) {
        ...
        CSExpression target = null == targetExpression
            ? new CSReferenceExpression(name) : new CSMemberReferenceExpression(targetExpression, name);
        CSMethodInvocationExpression mie = new CSMethodInvocationExpression(target);
        ...
        if (base != null && base.getReturnType() != method.getReturnType()
            ...
            pushExpression(new CSParenthesizedExpression(new CSCastExpression(mappedTypeReference(method
                .getReturnType()), mie)));
        else
            pushExpression(mie);
    }
    ...
    private void processMappedMethodInvocation(MethodInvocation node, IMethodBinding binding,
        ...
        if (0 == name.length()) {
            ...
            final CSExpression target = expression != null
                ? mapExpression(expression) : new CSThisExpression(); // see
        ...
        if (null == expression || isMappingToStaticMethod) {
            ...
            if (BindingUtils.isStatic(binding) && arguments.size() > 0) {
                ...
                target = new CSMemberReferenceExpression(parensIfNeeded(mapExpression(arguments.get(0))),
                                                         name);
        ...
        if (mapping.kind != MemberKind.Method) {
            ...
            if (binding != originalBinding && originalBinding.getReturnType() != binding.getReturnType()
                && !(node.getParent() instanceof ExpressionStatement))
                target = new CSParenthesizedExpression(new CSCastExpression(mappedTypeReference(originalBinding
            ...
            if (vb != null) {
                ...
                if (cls != null) {
                    if (isStaticImport(vb, _ast.imports())) {
                        ...
                        pushExpression(new CSMemberReferenceExpression(mappedTypeReference(cls), ident));
                        return false;
                    } else if (_configuration.separateInterfaceConstants() && cls.isInterface()
                               && ident.indexOf('.') == -1) {
                        pushExpression(new CSMemberReferenceExpression(mappedAuxillaryTypeReference(cls),
                                                                       ident));
                        return false;
                    }
                }
        ...
        if (_configuration.separateInterfaceConstants()) {
            ...
            if (binding instanceof ITypeBinding) {
                ...
                if (typeBinding.isInterface()) {
                    pushExpression(new CSMemberReferenceExpression(mappedAuxillaryTypeReference(typeBinding),
                                                                   name));
                    return;
                }
            }
    ...
    private IMethodBinding getOverridedMethod(IMethodBinding methodBinding) {
        ITypeBinding superclass = _ignoreExtends.value()
            ? resolveWellKnownType("java.lang.Object") : methodBinding.getDeclaringClass().getSuperclass();
        if (null != superclass) {
    ...
    CSVisibility mapVisibility(int modifiers, CSVisibility defaultVisibility) {
        ...
        if (Modifier.isProtected(modifiers)) {
            return _configuration.mapProtectedToProtectedInternal()
                ? CSVisibility.ProtectedInternal : CSVisibility.Protected;
        }
        if (Modifier.isPrivate(modifiers)) {
            return CSVisibility.Private;
        }
