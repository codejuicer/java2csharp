...
public class Environments {
    ...
    public static Environment newConventionBasedEnvironment(Object... bindings) {
        return newCachingEnvironmentFor(compose(newClosedEnvironment(bindings),
    ...
    private static final class ConventionBasedEnvironment implements Environment {
         ...
         * interface foo.bar.Baz default implementation foo.internal.bar.BazImpl
         *
        ...
        private <T> T resolve(Class<T> service) {
            ...
            if (null == binding) {
                throw new IllegalArgumentException("Cant find default implementation for "
                                                   + service.toString() + ": " + className);
            }
            return service.cast(binding);
        }
